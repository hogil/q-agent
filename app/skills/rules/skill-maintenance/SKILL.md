...
- 합성 난도 변경은 별도 data_root에 생성하고 기본 DB와 기존 결과를 보존한다. 2026-09-18 로컬 검색 누락 재현을 근거로 복합 WHERE, 복수 후보, 분산된 근거, 미래/미승인 문서를 검사한다. 사내 데이터 난도를 재현했다고 주장하지 않는다.
- 검색 개선 전에 golden 자체를 확인한다. 실제 SQL 후보 집합, 유효한 기간, required chunk의 사고/승인/시점, split 경계를 검증한다. reference_answer는 구체적 사실을 담고 일반적인 답변 지침으로 대신하지 않는다. 어휘가 겹치지 않는 실패도 숨기지 않는다.
