...
| Tool 실행 | app/plan_executor.py | 검사된 plan의 Tool 이름을 등록된 Python Adapter로 연결 |
...
agent.py가 이 호출 순서와 종료 한도를 제어한다. plan_executor.py는 검증된 한 단계의 실행만 담당한다.
...
plan_executor.py → runtime_factory.py → incident_tools.py로 이어진다.
...
plan_executor.execute_plan(output, context, adapters)는 plan 전체를 검사한 뒤
adapters[step['tool']](**step['arguments'])를 호출한다. 호출 코드는 사용자 정보와
검증된 scope를 바인딩한 Adapter를 등록해야 한다. 출력 JSON이 임의 함수를 실행하지는 않는다.
