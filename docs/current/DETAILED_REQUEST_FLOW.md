...
| ↓ | |
| **1계층: 사고 DB 우선 조회** | 질문에 따라 SQL 검색/집계, 사고 텍스트 Hybrid 또는 혼합 검색을 선택하고 사고 테이블으로 사고 범위를 확인한다. Hybrid 및 운영 집계 Adapter는 연결 예정이다. |
| ↓ | |
...
사고 DB 우선 조회 조건은 Tool 실행 코드가 검사한다. 최종 결과의 '사고 범위 표시'와 실행 전 범위 검증은 서로 다른 작업이다. DB만으로 답할 수 있는 질문은 2계층 Tool을 실행하지 않고 Judge로 진행한다. 어떤 Tool을 생략했는지는 조사 상태에 남긴다.
...
현재 Router 출력 형식 v2는 `intents`, `filters`, `plan`, `needs_skills`, `clarification`, `decision`, `stage`, `search_mode`, `limitations`이다. 세부 요구사항별 충족 상태는 Orchestrator 상태 계약으로 추가 구현한다. 현재 JSON 형식에 없는 필드가 이미 구현됐다고 가정하지 않는다.
...
사고 DB는 사람이 관리하는 정형 사고 테이블이다. 명칭은 **정형 사고 조회/집계(Structured Incident Retrieval / Analytics)**로 사용한다.
...
DB 조회 완료는 사고 테이블 전체를 읽었다는 뜻이 아니다. 질문에 필요한 조건과 대상 범위를 확인했다는 뜻이다. SQL 통계는 검색 상위 N개 사고가 아니라 전체 조건에 맞는 사고 테이블 집합을 대상으로 한다. 대규모 모집단은 모든 ID를 LLM에 전달하지 않고 승인된 필터와 기준시점으로 집계한다. 이 통계 범위 계약은 추가 구현 대상이며 현재 목록 scope의 사고 수 제한과 구분한다.
...
`aggregate_incidents`는 통계용 Tool 인터페이스이다. 현재 배열 집계 독립 데모는 있지만 전체 운영 SQL 집계 Adapter는 미구현이다. LLM은 숫자를 추산하지 않는다. 문서는 통계 산출 후 배경을 설명하는 근거로 붙이고 문서 검색 결과로 전체 사고 수를 계산하지 않는다.
...
| 모순/제한 | DB와 문서의 수치/시점 차이가 설명됐는가? 데이터 누락 여부와 권한 제한이 기록됐는가? |
...
| 분석 | 관측된 현상, 사고 테이블에 기록된 분석과 확정 원인, 가설 및 확인 필요사항 |
...
| 5 | Router → Tool → Judge → Answer Orchestrator | 우선 조회, 단일 Router 복귀, 근거 상태, 예산/타임아웃, 실제 LLM JSON 검증 구현 |
