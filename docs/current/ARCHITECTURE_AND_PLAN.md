...
```mermaid
flowchart TD
  U["Chat · 이미지 · Trend 의뢰"] --> R["Router + 필요한 공유 Skill"]
  R --> O["Orchestrator: 계약·권한·상태 검사"]
  O --> D["사고 DB 검색 또는 집계"]
  D --> S{"범위·충분성 검사"}
  S -->|조건 모호| R
  S -->|범위 확정| T["Lot · 기간시스템 · 이미지 · Trend Tool"]
  S -->|문서 근거 필요| H["사고 문서 + 두 Hybrid RAG Tool"]
  T --> E["출처를 가진 Evidence Bundle"]
  H --> E
  S -->|정형 조회 충족| E
  E --> A["Answer + Judge + 코드 검증"]
  A -->|근거 부족·한도 이내| O
  A -->|검증 통과·제한 명시| V["답변 · 표 · 이미지 · 조치 초안"]
  V --> P["별도 승인·실행 서비스"]
```
...
6. Answer가 DB 사실, 문서 근거, 이미지 관측, 추가 확인 필요사항을 나눈다. Judge와 코드 검증이 ID/수량/출처/질문 충족을 확인한다.
...
| 5. 답변·UI | Answer/Judge, 코드 gate, 조사 workspace와 통계 drill-down | 질문-근거 회귀 평가, 누락/불확실성 표시, 브라우저/권한 테스트 |
