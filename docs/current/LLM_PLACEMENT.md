...
그림의 Router는 맨 위 한 개뿐이다. Judge의 재조회 요청은 항상 이 Router로 돌아간다. Router는 현재 조사 상태에 따라 사고 검색 또는 후속 Tool을 계획한다. 사고 DB를 확인하기 전에는 후속 업무 Tool 실행을 허용하지 않는다. 단계별 재호출을 별도 Router 상자로 그리지 않는다.

추가 Tool은 Lot/Wafer 목록, 사고문서와 사내문서/Eng’r Inform Note RAG, 이미지, Trend, 기간시스템 조회다. 사고 DB 근거만으로 충분하면 추가 Tool 단계는 실행 없이 통과한다. 재조회 시 유효한 사고 범위는 유지할 수 있고 사고/조건 오류 또는 만료이면 사고 DB를 다시 조회한다.

Answer 아래 상자는 최종 결과 표시다. 조회 성공 여부와 확정된 사고 범위를 사용자에게 보여준다. 사고 범위 검증 자체는 후속 Tool 실행 전에 코드가 수행한다. 실패/미확정 결과를 성공/확정으로 표시하지 않는다. 별도 우선 조회 판단 상자는 도식에서 생략했다.

| 돌아오는 이유 | 돌아가는 위치 | 유지/재확인할 내용 |
|---|---|---|
| DB 조회 실패 또는 사고 후보 미확정 | 맨 위 Router | 조건/사고 선택을 확인. 접근권한 오류는 우회하지 않고 제한 표시 |
| Judge가 같은 사고의 근거 부족 판정 | 맨 위 Router | 유효한 사고 범위 유지, 부족한 근거만 추가 조회 |
| Judge가 잘못된 사고/조건 판정 또는 범위 만료 | 맨 위 Router | 기존 범위를 재사용하지 않고 사고 DB부터 재확인 |
...
이는 수정 시 검증 원칙이다. 일반 질문마다 전체 DB를 스캔하라는 뜻이 아니며, 사내 접근이 없는 개발 단계에서는 명확히 표시한 초안과 합성 회귀 검증을 진행할 수 있다. 실제 사고 테이블의 컬럼/관계/코드 의미를 확정하는 단계에서는 해당 실제 자료를 확인해야 한다.

| 수정 대상 | 확인할 실제 자료 |
|---|---|
| Schema Skill / 컬럼 매핑 | DDL, 컬럼 설명, 권한 내 대표 행, NULL 분포, 키 중복, 실제 관계 |
| 동의어 | 공식 조직/공정 코드와 적용 범위, 실제 사용자 표기, 유효기간 |
| Lot/Wafer 조회 | 같은 사고의 사고 테이블 수량과 연결 행, 중복/누락, 사고 영향/전체 구성 구분 |
...
2026-09-13 도식 보정: Router를 최상단의 단일 노드로 통합하고 Judge의 모든 재조회 화살표를 그 노드에 연결했다. Answer 아래에는 최종 조회 상태와 사고 범위 표시를 배치했다. 실행 전 사고 DB 검증 규칙은 유지한다.
