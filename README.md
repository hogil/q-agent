...
- 기본 화면은 통합 분석 격자입니다. 이상감지, Trend, 과거 Fab/EDS, Wafer 목록, 개별·합성 Map, 변경 이력, SEM/Overlay, 생산, 문서, 판정을 동시에 표시합니다. 작은 화면은 읽기 가능한 세로 배치로 전환하며, 상세 화면과 채팅은 별도로 유지합니다.
...
- 현재 Wafer는 Fab 단계로 EDS 결과를 생성하지 않습니다. Fab/EDS 비교는 같은 Item·Step의 과거 완료 이력에 설비·Recipe 조건을 적용하고, 현재 선택 시작 시각 이후 완료된 기록을 제외합니다. Fab 온도/Queue/가동률과 Yield/Bin 3/Bin 4를 선택해 산점도와 Trend를 봅니다. Pearson r은 `simple-statistics`로 계산하며 3쌍 미만 또는 변동이 없으면 N/A입니다. 상관관계를 원인으로 해석하지 않습니다.
