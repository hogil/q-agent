...
- 기본 화면은 브라우저 폭을 모두 사용하는 통합 분석 격자입니다. Item 표, Trend, Fab/EDS Corr, Wafer 목록, 개별·합성 Map, 변경 이력, SEM/Overlay, 생산, 문서, 분석·채팅을 동시에 표시합니다. 메뉴와 분석 계획은 겹쳐 여는 패널이며, 작은 화면은 세로 배치로 전환합니다.
- 데스크톱은 패널 모서리 드래그로 폭·높이를 조절합니다. 인접 패널끼리 공간을 배분하고 같은 행의 높이를 함께 바꾸므로 화면 밖으로 밀려나거나 패널이 겹치지 않습니다. 방향키 조절, Esc 취소, 상단 잠금·기본 배치 복원을 지원합니다. 크기는 이 브라우저에만 저장하며 모바일에서는 자동 배치를 사용합니다.
...
- 개별 Map은 Bin/CD/Overlay를 전환합니다. CD·Overlay ΔX/ΔY/크기는 nm 단위 측정점과 Delaunay 삼각형 내부 선형 보간값을 구분하며, 측정 범위 밖은 채우지 않습니다. 색상 범위는 Wafer 간 고정하고 영역 통계는 원래 측정점만 셉니다. 현재 측정점은 `D:\project\q-agent\web\src\metrologyMap.ts`의 합성 fixture이며 Raw 파일·측정 시스템·Agent Tool 연결은 아직 없습니다. 전체 Map은 여전히 Bin Flag 합성이며 CD 평균 Map이 아닙니다.
