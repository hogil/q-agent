...
### 첫 접속: LLM 분석 결과와 근거 이미지

기본 주소(`/`)에서는 가장 최근에 완료된 LLM 분석을 바로 표시합니다. 온도 변동 수치, 불량 의심 근거, 비교 SEM과 과거 참조, 점검·EDS 확인 계획을 함께 봅니다. 이미지는 확대할 수 있으며 `분석 보드`와 같은 대화방의 `추가 질문`으로 이어집니다. 재접속은 저장된 결과를 읽으며 LLM을 다시 호출하지 않습니다. 저장된 분석이 없거나 대화방 주소로 직접 접속하면 기존 작업실을 엽니다.

![첫 화면: 저장된 LLM 분석, Trend 수치, 선택 Wafer와 비교 SEM, 과거 참조](docs/images/startup-analysis.png)
